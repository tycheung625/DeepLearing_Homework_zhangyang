...
# 打印预测值和对应的真实值
for i in range(30):
    day = prediction_data.iloc[i]['days_since_reference']
    max_temp_real = prediction_data.iloc[i]['最高温度']
    min_temp_real = prediction_data.iloc[i]['最低温度']
    max_temp_predict = predictions[i, 0]
    min_temp_predict = predictions[i, 1]
    
    print(f"第 {i + 1} 天：预测最高温度={max_temp_predict}, 实际最高温度={max_temp_real},预测最低温度={min_temp_predict},  实际最低温度={min_temp_real}")
